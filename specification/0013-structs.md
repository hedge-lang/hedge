...
Capabilities belong to the binding rather than to the field: `let write s` makes
...
borrowed at once, including more than one `&write`:

```hedge
let write p = Point { x: 0, y: 0 };
let a = &write p.x;
let b = &write p.y;   // distinct place, no conflict
```

Three boundaries qualify this. A borrow of a field excludes any overlapping borrow,
so `&write s.a` excludes both `&s` and any other borrow of `s.a`. A `&self` or
`&write self` method borrows the whole struct, because disjointness is not inferred
...
`&write arr[0]` and `&write arr[1]` conflict, since the compiler cannot prove the
indices differ.
...
capability. There is no `pub write`, so to expose a field for reading while keeping
...
impl Point {
  ...
  fn shift(&write self, dx: i32) { self.x = self.x + dx }
}
```
...
| `&write self` | exclusive (write) borrow     |

There is no `bind self`. Call sites auto-reference the receiver, so `p.x()` takes
`&p`, `p.shift(1)` takes `&write p` and therefore requires `p` to hold `write`, and
