...
export const CROSS_DOMAIN_CONFORMANCE_RULES: readonly ConformanceRule[] = [
  ...
  },
  {
    ...
    testIds: [
      ...
      "rejects moving an element out of a fixed-size-array parameter typed with a declared generic parameter, since it is not provably Copy",
      "rejects reusing a generic-typed value more than once in a fixed-size array literal, since it is not provably Copy",
      "rejects a repeat-form array literal whose generic-typed value is not provably Copy",
    ],
  },
  {
    id: "DROP-DETERMINISM",
  ...
  },
  {
    ...
    testIds: [
      ...
      "resolves a generic type parameter used as a fixed-size array's element type",
      "resolves a generic function's own type parameter used as an array element type in a return type",
      "rejects a generic type parameter used as an array element type behind a reference",
  ...
  },
  {
    ...
    testIds: [
      ...
      "does not cascade an unsolved-variable diagnostic when a bare type parameter's only occurrence is an array literal with an unresolved element",
      ...
      "infers a generic parameter from a fixed-size array argument",
      "substitutes a fixed-size-array return type from argument-inferred bindings",
      "rejects a fixed-size array argument whose length does not match the declared element-type parameter",
      "reports conflicting concrete element types inferred for the same type parameter across two fixed-size-array arguments",
    ],
  },
  {
  ...
  {
    id: "GENERIC-NAMED-FIELD-CONSTRUCTION-INFERENCE",
    description:
      "A generic struct or enum-variant constructed with named fields unifies its type parameters from the explicitly-listed field values, the same way positional construction does - a ..base spread never seeds or is checked against inference, and a shorthand field participates via its own binding's type",
    specRefs: ["0015-generics-and-traits.md"],
    testIds: [
      "infers a type parameter from a single named field's value, with no annotation",
      ...
      "coerces an unsuffixed literal named field value against the already-resolved concrete type",
      "resolves an otherwise-unsolved named-field type parameter to its declared default",
      "reports a named-field type parameter as unsolved when no listed field references it and it has no default",
      "does not seed inference from a field supplied only via a ..base spread",
      ...
      "does not cascade an unsolved-variable diagnostic when a bare named-field type parameter's only occurrence is an array literal with an unresolved element",
      ...
      "lets an explicit turbofish resolve a named-field struct construction's type parameter",
      "reports a conflict when a named-field struct construction's turbofish disagrees with a field value",
      "infers a type parameter from a single named field's value on an enum variant, with no annotation",
      "infers two independent type parameters from two named fields on an enum variant",
      "reports a conflicting inference across two named fields on an enum variant",
      "resolves an otherwise-unsolved enum-variant type parameter to the enum's declared default",
      "reports an enum generic parameter unused by the constructed named-fields variant as unsolved",
      "does not seed inference from a field supplied only via a ..base spread on an enum-variant construction",
      "lets an explicit turbofish resolve a named-field enum-variant construction's type parameter",
    ],
  },
  {
